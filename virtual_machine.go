...
func (vm *HyperVVirtualMachine) VM() (*virtualsystem.VirtualMachine, error) {
	wmiInstance, err := wmictl.GetWmiInstanceFromPath(wmictl.VirtualizationV2, vm.instancePath)
	if err != nil {
		return nil, err
	}
...
func (vm *HyperVVirtualMachine) ModifySpec(options ...Option) (ok bool, err error) {
	...
	mgmt, err := wmictl.NewLocalVirtualSystemManagementService()
	if err != nil {
		return false, err
	}
	// 修改 CPU 核心数
...
func GetVirtualMachineByName(vmName string) (*HyperVVirtualMachine, error) {
	vm, err := wmictl.GetVirtualMachineByVMName(vmName)
	if err != nil {
		return nil, err
	}
